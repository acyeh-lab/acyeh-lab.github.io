#!/usr/bin/env python3
"""Parse the Yeh lab antibody xlsx files into lab-reagents-data.json for flow.html.

Reads:  /fh/fast/yeh_a/grp/reference_data/hill_reagents/*.xlsx
...
REAGENT_DIR = Path("/fh/fast/yeh_a/grp/reference_data/hill_reagents")
...
SECTION_PATTERNS = [
    # Purified / unconjugated sections carry no fluorochrome. Must come first so
    # "Unconjugated" is never caught by a later pattern; rows below are skipped.
    (r"^\s*(un|non[\s-]*)conjugat",  "_UNCONJUGATED_"),
    ...
    (r"APC[\s/-]?Fire\b",            "APC-Fire 750"),      # bare "APC-Fire" = the 750
    (r"APC[\s/-]?Cy7",               "APC-Cy7"),
    (r"APC[\s/-]?H7",                "APC-H7"),
    ...
    # Yellow-green laser.  Cy separators vary in the sheets: "PE/Cy5",
    # "PE-Cy5", "PE/Cy-5" all occur, so allow a separator after "Cy" too.
    (r"PE[\s/-]?Cy[\s-]?5\.5",       "PE-Cy5.5"),
    (r"PE[\s/-]?Cy[\s-]?5",          "PE-Cy5"),
    (r"PE[\s/-]?Cy[\s-]?7",          "PE-Cy7"),
    ...
    # BB515/BB700 are their own dyes (BD Horizon Brilliant Blue), reported under
    # their real names. They share a detector with FITC / PerCP-Cy5.5 but have
    # different spectra, so calling them FITC/PerCP-Cy5.5 mislabels what you
    # would actually order. Their spectra live in flow-spectra-data.json.
    (r"\bBB515\b",                   "BB515"),
    (r"\bBB700\b",                   "BB700"),
    (r"\bFITC\b",                    "FITC"),
    ...
    (r"\bV450\b",                    "Pacific Blue"),     # BD V450 == Pacific Blue dye

    # UV laser. These sections are titled "BUV395 Conjugated antibodies" etc.,
    # almost all of them in COLUMN 2 of the sheet. Must precede the generic
    # "UV conjugate" container pattern below.
    (r"\bBUV\s*395\b",               "BUV395"),
    (r"\bBUV\s*496\b",               "BUV496"),
    (r"\bBUV\s*563\b",               "BUV563"),
    (r"\bBUV\s*605\b",               "BUV605"),
    (r"\bBUV\s*615\b",               "BUV615"),
    (r"\bBUV\s*661\b",               "BUV661"),
    (r"\bBUV\s*737\b",               "BUV737"),
    (r"\bBUV\s*805\b",               "BUV805"),
    ...
    (r"Alexa\s*Fluor\s*568\s*conjugated\s*tetram", "Alexa Fluor 568"),
...
# A cell that is JUST a dye name, used to spot sub-section headers that omit the
# word "conjugated" (the sheets are full of bare "Brilliant violet 421" rows).
# Anchored at both ends so real targets ("CD3", "TCR Vα7.2") can never match.
FLUOR_LABEL_RE = re.compile(
    r"^\s*(brilliant\s*(ultra[\s-]*)?violet\s*\d{3}|brilliant\s*(blue|uv)\s*\d{3}|"
    r"BUV\s*\d{3}|BV\s*\d{3}|BB\s*\d{3}|V\d{3}|"
    r"alexa\s*fluor\s*\d{3}|AF\d{3}|"
    r"APC([\s/-]\S+)?|PE([\s/-]\S+)?|PerCP([\s/-]\S+)?|FITC|"
    r"eFluor\s*\d{3}|Pacific\s*(Blue|Orange)|Super\s*Bright\s*\d{3}|"
    r"Spark\s*\w*\s*\d{3}|cFluor\s*\S+|Zombie\s*\w+)\s*$",
    re.IGNORECASE,
)

# Section headers we recognised as headers but could not map to a fluorochrome.
# Collected rather than silently ignored -- an unmapped header used to mean its
# rows inherited the PREVIOUS dye, which is how ~136 reagents were mislabelled.
UNMAPPED_BANNERS = []
...
def match_section(text):
    ...
    if not text:
        return None
    # The sheets spell the same dyes two ways: "BV421" and "Brilliant violet
    # 421", "BUV395" and "Brilliant Ultra Violet 395". Normalize the spelled-out
    # form to the short form so one set of patterns covers both. Ultra-violet
    # must be rewritten before violet or "Brilliant ultra violet 395" would
    # become "Brilliant ultra BV395".
    norm = re.sub(r"brilliant\s*ultra[\s-]*violet\s*", "BUV", text, flags=re.I)
    norm = re.sub(r"brilliant\s*uv\s*",                "BUV", norm, flags=re.I)
    norm = re.sub(r"brilliant\s*violet\s*",            "BV",  norm, flags=re.I)
    norm = re.sub(r"brilliant\s*blue\s*",              "BB",  norm, flags=re.I)
    ...
    with zipfile.ZipFile(xlsx_path) as z:
        ...
        for sheet_name, sheet_target in sheets:
            ...
            for r in sorted(rows.keys()):
                ...
                # A row is a BANNER only if it carries no reagent data: no cat#
                # in col3, and not a target+clone pair. Tetramer sheets legally
                # have col0-only data rows, so there the old keyword rule stands.
                is_data_row = bool(col3) or (bool(col0) and bool(col1))
                sec = None
                banner_like = None
                for banner_text in (col0, col1, col2):
                    if not banner_text or len(banner_text) > 80:
                        continue
                    if banner_text in ("Antibody", "Tetramer", "Clone", "Isotype"):
                        continue
                    keyworded = re.search(
                        r"(conjugat|UV conjugate|Streptavidin|Annexin|Biotin|tetramer)",
                        banner_text, re.I)
                    # Sub-sections are often just the bare dye name with no
                    # "conjugated" -- e.g. "Brilliant violet 421". Accept those
                    # when the row holds no reagent data.
                    bare_dye = (not is_data_row and not is_tetramer_file
                                and FLUOR_LABEL_RE.match(banner_text))
                    if not keyworded and not bare_dye:
                        continue
                    if is_data_row and not keyworded:
                        continue
                    ...
                    if candidate:
                        ...
                        break
                    if not is_data_row:
                        banner_like = banner_text
                if sec:
                    current_fluor = sec
                    in_header_area = True
                    continue
                if banner_like is not None and not is_tetramer_file:
                    # A section header we cannot map. Do NOT let the rows below
                    # inherit the previous fluorochrome -- that is precisely how
                    # ~136 Brilliant Violet / BUV605 reagents were published as
                    # PerCP-Cy5.5. Clear the section and record it so the
                    # pattern can be added.
                    UNMAPPED_BANNERS.append((xlsx_path.name, sheet_name, r, banner_like))
                    current_fluor = None
                    in_header_area = True
                    continue
...
def main():
    ...
    out = {
        "_meta": {
            ...
            "note":       "Built by build_reagents.py from the Yeh lab xlsx inventory files. "
    ...
    if UNMAPPED_BANNERS:
        print(f"\nWARNING: {len(UNMAPPED_BANNERS)} section header(s) could not be "
              f"mapped to a fluorochrome. Their rows were SKIPPED rather than "
              f"inheriting the previous dye. Add a SECTION_PATTERNS entry for each:",
              file=sys.stderr)
        for fname, sheet, row, text in UNMAPPED_BANNERS:
            print(f"  {fname} [{sheet}] r{row}: {text!r}", file=sys.stderr)
